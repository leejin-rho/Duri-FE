...
const ReplyFormDetail = ({
  ...
  const handleChange = (
    key: keyof QuotationFormData['priceDetail'],
    value: number,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  return (
    <Flex direction="column" gap={24} padding="24px 0">
      <Flex direction="column" align="flex-start" gap={8}>
        <Text typo="Title2" colorCode={theme.palette.Black}>
          미용 메뉴
        </Text>
        <Flex direction="column" padding="20px" gap={32}>
          ...
          {groomingMenu.length > 0 && (
            <AmountInput
              ...
              menu={groomingMenu}
              value={groomingPrice}
              onChange={(value) => handleChange('groomingPrice', value)}
            />
          )}
          ...
          {specialCare.length > 0 && (
            <AmountInput
              title="스페셜 케어"
              menu={specialCare}
              value={specialCarePrice}
              onChange={(value) => handleChange('specialCarePrice', value)}
            />
          )}
          ...
          {designCut.length > 0 && (
            <AmountInput
              ...
              menu={designCut}
              value={designPrice}
              onChange={(value) => handleChange('designPrice', value)}
            />
          )}
          ...
          {otherRequests && (
            ...
            //   menu={otherRequests}
            //   value={customPrice}
            //   onChange={(value) => handleChange('customPrice', value)}
            // />
            <Flex
              direction="column"
              justify="flex-start"
            ...
            >
              ...
              <Flex direction="column" align="flex-start" gap={12}>
                ...
                <Flex justify="flex-end">
                  <Flex gap={8} align="flex-end" width={174}>
                    <TextField
                      value={customPrice}
                      onChange={(e) => {
                        handleChange('customPrice', Number(e.target.value));
                      }}
                      widthPer="100%"
