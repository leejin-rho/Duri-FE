...
const ReplyForm = ({
  ...
  const [reservationTimeList, setReservationTimeList] =
    useState<TimeType>(defaultTimeList);

  const handleReserve = (key: string, value: boolean) => {
  ...
  useEffect(() => {
    const { isEmpty, isCountinuous, startDateTime, endDateTime } =
      checkContinuousTime(reservationTimeList, requestDay);
  ...
  return (
    <Flex direction="column" gap={24} padding="24px 0">
      ...
      <Flex direction="column" align="flex-start" gap={8}>
        ...
        <Text typo="Label3" colorCode={theme.palette.Gray400}>
          희망 예약 시간을 보고 사장님이 미용가능한 시간을 지정해주세요.
        </Text>
        <TimeTableGroomer
          timeList={timeList}
          selectedTimeList={selectedTimeList}
